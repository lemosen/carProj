...
export class FormExpressTemplateComponent implements OnInit, OnChanges {
  ...
  buildForm(): void {
    this.commonForm1 = this.fb.group({
      arryControl: this.fb.array([this.fb.group({
        ...
        printWidth: [null, Validators.compose([Validators.required, Validators.maxLength(15)])],
        printHigh: [null, Validators.compose([Validators.required, Validators.maxLength(15)])],
  ...
  setBuildFormValue(expressTemplate: ExpressTemplate) {
    this.commonForm1.patchValue({
      arryControl:  [
        {
        templateName: expressTemplate.templateName,
        printWidth: expressTemplate.printWidth,
        printHigh: expressTemplate.printHigh,
        state: expressTemplate.state,
      }],
    });
  }
