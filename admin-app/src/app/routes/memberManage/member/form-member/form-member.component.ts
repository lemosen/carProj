...
export class FormMemberComponent implements OnInit, OnChanges {
  ...
  constructor(private fb: FormBuilder, public memberLevelService: MemberLevelService, private location: Location, public msgSrv: NzMessageService,public communityService:CommunityService) {
    this.buildForm();
  }
  ...
  setBuildFormValue(member: Member) {
    this.commonForm.setValue({
      ...
      community : {
        id:member.community ? member.community.id : null,
        address:member.community ? member.community.address : '请选择',
      }
    });
    let position = []
