...
@Component({
  selector: 'list-seckill',
  templateUrl: './list-seckill.component.html',
  styleUrls: ['./list-seckill.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ListSeckillComponent implements OnInit {
  ...
  constructor(public route: ActivatedRoute, public router: Router, private seckillService: SeckillService, public msg: NzMessageService,
  ...
  getData() {
    this.loading = true;
    this.seckillService.query(this.pageQuery).subscribe(response => {
  ...
  remove(seckillId) {
    this.seckillService.removeById(seckillId).subscribe(response => {
