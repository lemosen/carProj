...
export class ModalSelecetComponent {
  ...
  checkAll(value: boolean): void {
    ...
    if (value) {
      let select = []
      if (this.allCheckedSelect != null) {
        this.spliceAllCheckedSelect();
        this.allCheckedSelect.forEach(e => {
          select.push(e)
        })
      }
      this._anys.forEach(e => {
      ...
      if (this.allCheckedSelect != null) {
        this.spliceAllCheckedSelect();
        this.select = this.allCheckedSelect;
      } else {
        this.select = []
      }
    }
    this.allCheckedSelect = this.select
    this.changeResultName();
  }

  /**
   * 去除 allCheckedSelect 集合中 _anys 集合数据
   */
  spliceAllCheckedSelect(){
    this._anys.forEach(e => {
      this.allCheckedSelect.forEach((e1, index) => {
        if (e.id == e1.id) {
  ...
  filterRe(arr, field) {
    let obj = {}, arr1 = [];
    arr.forEach(e => {
      obj[e[field]] = obj[e[field]] ? obj[e[field]] : e
    })
    for (let objKey in obj) {
      arr1.push(obj[objKey])
    }
    return arr1;
  }
  ...
  getData() {
    ...
    if (this.custom) {
      this.baseService['query'](this.pageQuery).subscribe(response => {
        ...
        this.multiSelect(this.numberOfElements);
      }, error => {
        //this._loading = false;
      });
    } else {
  ...
  getSelect(item, id) {
    ...
    if (item != null) {
      if (!this.isFirst) {
        if(this.allCheckedSelect !=null && this.allCheckedSelect.length != 0){
          this.select = this.allCheckedSelect;
        }
  ...
  private multiSelect(numberOfElements) {
    if (this.isMulti) {
      let isPageChecked = 0;
      this._anys.forEach(e => {
        e.checked = false;
      ...
      if (isPageChecked == numberOfElements) {
